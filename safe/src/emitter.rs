...
use crate::types::{
    yaml_break_t::{YAML_ANY_BREAK, YAML_CRLN_BREAK, YAML_CR_BREAK, YAML_LN_BREAK},
    ...
    yaml_error_type_t::{YAML_EMITTER_ERROR, YAML_MEMORY_ERROR},
    yaml_event_t,
    yaml_event_type_t::{
    ...
    yaml_scalar_style_t,
    yaml_scalar_style_t::{
        YAML_ANY_SCALAR_STYLE, YAML_DOUBLE_QUOTED_SCALAR_STYLE, YAML_FOLDED_SCALAR_STYLE,
...
unsafe fn FLUSH(emitter: *mut yaml_emitter_t) -> Success {
    if (*emitter).buffer.pointer.wrapping_offset(5_isize) < (*emitter).buffer.end {
        ...
        yaml_emitter_flush_impl(emitter)
    }
}
...
/// function fails.
pub(crate) unsafe fn yaml_emitter_emit_impl(
    mut emitter: *mut yaml_emitter_t,
...
pub unsafe extern "C" fn yaml_emitter_emit(
    ...
    ffi::int_boundary(|| unsafe {
        ...
        if yaml_emitter_emit_impl(emitter, event).ok {
            crate::OK
        } else {
            crate::FAIL
        }
...
unsafe fn yaml_emitter_need_more_events(emitter: *mut yaml_emitter_t) -> Success {
    ...
    let accumulate = match (*(*emitter).events.head).r#type {
...
unsafe fn yaml_emitter_emit_stream_start(
    ...
    if (*event).r#type == YAML_STREAM_START_EVENT {
...
unsafe fn yaml_emitter_emit_document_start(
    ...
    if (*event).r#type == YAML_DOCUMENT_START_EVENT {
        ...
        if !first || flag((*emitter).canonical) {
            implicit = 0;
        }
        if (!(*event).data.document_start.version_directive.is_null()
        ...
        (*emitter).open_ended = 0;
        return OK;
    } else if (*event).r#type == YAML_STREAM_END_EVENT {
        ...
        if yaml_emitter_flush_impl(emitter).fail {
            return FAIL;
        }
...
unsafe fn yaml_emitter_emit_document_end(
    ...
    if (*event).r#type == YAML_DOCUMENT_END_EVENT {
        if yaml_emitter_write_indent(emitter).fail {
            return FAIL;
        }
        if !flag((*event).data.document_end.implicit) {
            if yaml_emitter_write_indicator(
                emitter,
                b"...\0" as *const u8 as *const libc::c_char,
                true,
...
unsafe fn yaml_emitter_emit_flow_sequence_item(
    ...
    if (*event).r#type == YAML_SEQUENCE_END_EVENT {
        ...
        *fresh13 -= 1;
        ...
        if flag((*emitter).canonical) && !first {
            if yaml_emitter_write_indicator(
                emitter,
                b",\0" as *const u8 as *const libc::c_char,
                false,
    ...
    if flag((*emitter).canonical) || (*emitter).column > (*emitter).best_width {
        if yaml_emitter_write_indent(emitter).fail {
            return FAIL;
        }
    }
...
unsafe fn yaml_emitter_emit_node(
    ...
    (*emitter).root_context = bool_flag(root);
    (*emitter).sequence_context = bool_flag(sequence);
    (*emitter).mapping_context = bool_flag(mapping);
    (*emitter).simple_key_context = bool_flag(simple_key);
    match (*event).r#type {
...
unsafe fn yaml_emitter_emit_alias(
    ...
    if flag((*emitter).simple_key_context) {
...
unsafe fn yaml_emitter_check_empty_sequence(emitter: *mut yaml_emitter_t) -> bool {
    ...
    (*(*emitter).events.head).r#type == YAML_SEQUENCE_START_EVENT
        && (*(*emitter).events.head.wrapping_offset(1_isize)).r#type == YAML_SEQUENCE_END_EVENT
}
...
unsafe fn yaml_emitter_check_empty_mapping(emitter: *mut yaml_emitter_t) -> bool {
    ...
    (*(*emitter).events.head).r#type == YAML_MAPPING_START_EVENT
        && (*(*emitter).events.head.wrapping_offset(1_isize)).r#type == YAML_MAPPING_END_EVENT
}
...
unsafe fn yaml_emitter_check_simple_key(emitter: *mut yaml_emitter_t) -> bool {
    ...
    match (*event).r#type {
        ...
        YAML_SCALAR_EVENT => {
            if flag((*emitter).scalar_data.multiline) {
                return false;
            }
...
unsafe fn yaml_emitter_select_scalar_style(
    ...
    if flag((*emitter).simple_key_context) && flag((*emitter).scalar_data.multiline) {
        style = YAML_DOUBLE_QUOTED_SCALAR_STYLE;
    }
    if style == YAML_PLAIN_SCALAR_STYLE {
        if (*emitter).flow_level != 0 && !flag((*emitter).scalar_data.flow_plain_allowed)
            || (*emitter).flow_level == 0 && !flag((*emitter).scalar_data.block_plain_allowed)
        {
            style = YAML_SINGLE_QUOTED_SCALAR_STYLE;
        }
        if (*emitter).scalar_data.length == 0
            && ((*emitter).flow_level != 0 || flag((*emitter).simple_key_context))
        {
            style = YAML_SINGLE_QUOTED_SCALAR_STYLE;
        }
        if no_tag && !flag((*event).data.scalar.plain_implicit) {
            style = YAML_SINGLE_QUOTED_SCALAR_STYLE;
        }
    }
    if style == YAML_SINGLE_QUOTED_SCALAR_STYLE {
        if !flag((*emitter).scalar_data.single_quoted_allowed) {
            style = YAML_DOUBLE_QUOTED_SCALAR_STYLE;
        }
    }
    if style == YAML_LITERAL_SCALAR_STYLE || style == YAML_FOLDED_SCALAR_STYLE {
        if !flag((*emitter).scalar_data.block_allowed)
            || (*emitter).flow_level != 0
            || flag((*emitter).simple_key_context)
        {
            style = YAML_DOUBLE_QUOTED_SCALAR_STYLE;
        }
    }
    if no_tag && !flag((*event).data.scalar.quoted_implicit) && style != YAML_PLAIN_SCALAR_STYLE {
        let fresh46 = addr_of_mut!((*emitter).tag_data.handle);
        *fresh46 = b"!\0" as *const u8 as *const libc::c_char as *mut yaml_char_t;
        (*emitter).tag_data.handle_length = 1_usize;
    }
...
unsafe fn yaml_emitter_process_anchor(emitter: *mut yaml_emitter_t) -> Success {
    ...
    if yaml_emitter_write_indicator(
        emitter,
        if flag((*emitter).anchor_data.alias) {
            b"*\0" as *const u8 as *const libc::c_char
        } else {
            b"&\0" as *const u8 as *const libc::c_char
        },
...
unsafe fn yaml_emitter_analyze_anchor(
    ...
    *fresh47 = string.start;
    ...
    (*emitter).anchor_data.alias = bool_flag(alias);
    OK
}
...
unsafe fn yaml_emitter_analyze_scalar(
    ...
    *fresh51 = value;
    ...
    while string.pointer != string.end {
        ...
        if !IS_PRINTABLE!(string) || !IS_ASCII!(string) && !flag((*emitter).unicode) {
    ...
    (*emitter).scalar_data.multiline = bool_flag(line_breaks);
    ...
    match (*event).r#type {
        ...
        YAML_SEQUENCE_START_EVENT => {
            ...
            if !(*event).data.sequence_start.tag.is_null()
                && (flag((*emitter).canonical) || !flag((*event).data.sequence_start.implicit))
            {
        ...
        YAML_MAPPING_START_EVENT => {
            ...
            if !(*event).data.mapping_start.tag.is_null()
                && (flag((*emitter).canonical) || !flag((*event).data.mapping_start.implicit))
            {
...
unsafe fn yaml_emitter_write_indent(mut emitter: *mut yaml_emitter_t) -> Success {
    ...
    if !flag((*emitter).indention)
        || (*emitter).column > indent
        || (*emitter).column == indent && !flag((*emitter).whitespace)
    {
...
unsafe fn yaml_emitter_write_indicator(
    ...
    if need_whitespace && !flag((*emitter).whitespace) {
        if PUT(emitter, b' ').fail {
            return FAIL;
        }
    }
    ...
    (*emitter).whitespace = bool_flag(is_whitespace);
    (*emitter).indention = bool_flag(flag((*emitter).indention) && is_indention);
    OK
}
...
unsafe fn yaml_emitter_write_plain_scalar(
    ...
    let mut string = STRING_ASSIGN!(value, length);
    if !flag((*emitter).whitespace) && (length != 0 || (*emitter).flow_level != 0) {
        if PUT(emitter, b' ').fail {
            return FAIL;
        }
    }
...
unsafe fn yaml_emitter_write_double_quoted_scalar(
    ...
    while string.pointer != string.end {
        if !IS_PRINTABLE!(string)
            || !flag((*emitter).unicode) && !IS_ASCII!(string)
            || IS_BOM!(string)
            || IS_BREAK!(string)
            || CHECK!(string, b'"')
            || CHECK!(string, b'\\')
